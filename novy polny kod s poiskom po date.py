    ...
    while True:
        ...
        for note in notes_to_check:
            ...
            if not note['reminder_24h_sent'] and time_diff <= timedelta(days=1) and time_diff > timedelta(hours=1):
                try:
                    await bot.send_message(user_id,
                                           f"Напоминание (24 часа): \"{note_text}\" в категории \"{note_type}\" запланировано на {note_date_str} {note_time_str}")
            ...
            # Напоминание за 1 час
                ...
                try:
                    await bot.send_message(user_id,
                                           f"Напоминание (1 час): \"{note_text}\" в категории \"{note_type}\" запланировано на {note_date_str} {note_time_str}")
