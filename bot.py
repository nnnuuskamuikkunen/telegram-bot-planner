...
from database import init_db, add_note, get_user_notes, delete_note, get_notes_for_reminders, edit_notes
from database import mark_reminder_sent, get_note_by_id, get_notes_by_type, get_notes_by_date
...
async def show_notes_handler(callback: types.CallbackQuery):
    await callback.message.edit_text("Поиск заметок:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
@router.message(F.text.regexp(r'[a-z]+([\.-]*[a-z]*)*@gmail.com'), AddNoteStates.synchronize)
async def process_note_text(message: types.Message, state: FSMContext):
    """Обрабатывает почту и запрашивает категорию"""
    mail = message.text.strip().lower()
    user_data = await state.get_data()
    note = get_note_by_id(DATABASE_NAME, user_data["note_id"], message.from_user.id)
    date_time = f'{note["note_date"]} {note["note_date"]}'
    #await book_timeslot(note["note_text"], date_time, mail)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Посмотреть все заметки", callback_data="list_notes")],
            [InlineKeyboardButton(text="В главное меню", callback_data="back_to_main")]
        ])
...
async def process_calendar_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data[0] == "select":
        # Выбран день
        year, month, day = int(data[4]), int(data[3]), int(data[2])
...
async def  handle_date_search(callback: types.CallbackQuery, search_date: date):
    ...
    user_id = callback.from_user.id
    notes = await get_notes_by_date(DATABASE_NAME, user_id, search_date.strftime("%d-%m-%Y"))
