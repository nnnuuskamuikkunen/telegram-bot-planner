...
async def get_user_notes(db_name: str, user_id: int):
    ...
    async with aiosqlite.connect(db_name) as db:
        ...
        cursor = await db.cursor()
        await cursor.execute(
            'SELECT id, note_text, note_date, note_time FROM notes WHERE user_id = ? ORDER BY note_date, note_time',
            (user_id,))
...
async def get_notes_for_reminders(db_name: str):
    ...
    async with aiosqlite.connect(db_name) as db:
        ...
        await cursor.execute(
            'SELECT id, user_id, note_text, note_type, note_date, note_time, task_complete, reminder_24h_sent, reminder_1h_sent FROM notes WHERE reminder_24h_sent = 0 OR reminder_1h_sent = 0')
        notes = await cursor.fetchall()
        return notes
